...
class AttendanceQrValidationServiceTest extends TestCase
{
    ...
    public function test_it_rejects_expired_qr_token(): void
    {
        $office = $this->createOfficeLocation();
        $this->createAttendanceQrToken($office, [
            'token' => 'EXPIRED-QR-001',
            'expired_at' => Carbon::parse('2026-03-27 08:59:00', 'Asia/Jakarta'),
        ]);
        ...
        $this->expectException(ExpiredQrTokenException::class);

        $service->validateForOffice(
            userId: 99,
            expectedOfficeLocationId: $office->id,
            rawToken: 'EXPIRED-QR-001',
            now: Carbon::parse('2026-03-27 09:00:00', 'Asia/Jakarta'),
        );
    }
}
