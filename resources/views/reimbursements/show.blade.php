...
@section('title', 'Detail Reimbursement #' . $reimbursement->id)
@section('header', 'Detail Reimbursement')

@section('content')
<div class="max-w-4xl mx-auto">
    <nav class="mb-4 text-xs text-gray-500">
        <a href="{{ route('reimbursements.index') }}" class="hover:text-primary-600">Reimbursement</a>
        <span class="mx-1">/</span><span class="text-gray-700">Detail #{{ $reimbursement->id }}</span>
    </nav>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div class="space-y-6 lg:col-span-2">
            <article class="overflow-hidden bg-white border border-gray-100 shadow-sm rounded-2xl">
                <header class="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gray-50">
                    <h2 class="font-semibold text-gray-800">Reimbursement #{{ $reimbursement->id }}</h2>
                    @php
                        $s1 = $reimbursement->status_1; $s2 = $reimbursement->status_2;
                ...
                <div class="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    <div><p class="text-xs text-gray-400 mb-0.5">Karyawan</p>
                        <p class="font-medium">{{ $reimbursement->employee?->name }}</p>
                        <p class="text-xs text-gray-500">{{ $reimbursement->employee?->email }}</p></div>
                    <div><p class="text-xs text-gray-400 mb-0.5">Tipe</p>
                        <p class="font-medium">{{ $reimbursement->type?->name ?? '–' }}</p></div>
                    <div><p class="text-xs text-gray-400 mb-0.5">Total</p>
                        <p class="font-medium text-lg text-gray-900">Rp {{ number_format($reimbursement->total, 0, ',', '.') }}</p></div>
                    <div><p class="text-xs text-gray-400 mb-0.5">Tanggal</p>
                        <p class="font-medium">{{ \Carbon\Carbon::parse($reimbursement->date)->format('d F Y') }}</p></div>

                    {{-- Stage 1 --}}
                    <div><p class="text-xs text-gray-400 mb-0.5">Status Tahap 1 (Team Leader)</p>
                        <span class="text-xs font-medium {{ $reimbursement->status_1 === 'approved' ? 'text-emerald-600' : ($reimbursement->status_1 === 'rejected' ? 'text-rose-500' : 'text-amber-600') }}">
                            {{ ucfirst($reimbursement->status_1) }}</span>
                        @if($reimbursement->note_1)
                        <p class="mt-0.5 text-xs text-gray-500">Catatan: {{ $reimbursement->note_1 }}</p>
                        @endif</div>

                    {{-- Stage 2 --}}
                    <div><p class="text-xs text-gray-400 mb-0.5">Status Tahap 2 (Manager)</p>
                        <span class="text-xs font-medium {{ $reimbursement->status_2 === 'approved' ? 'text-emerald-600' : ($reimbursement->status_2 === 'rejected' ? 'text-rose-500' : 'text-amber-600') }}">
                            {{ ucfirst($reimbursement->status_2) }}</span>
                        @if($reimbursement->note_2)
                        <p class="mt-0.5 text-xs text-gray-500">Catatan: {{ $reimbursement->note_2 }}</p>
                        @endif</div>

                    @if($reimbursement->invoice_path)
                    <div class="sm:col-span-2">
                        <p class="text-xs text-gray-400 mb-1">Bukti / Invoice</p>
                        <a href="{{ Storage::url($reimbursement->invoice_path) }}" target="_blank"
                            class="inline-flex items-center gap-2 px-3 py-1.5 text-xs text-primary-700 bg-primary-50 rounded-lg hover:bg-primary-100">
            ...
            <article class="overflow-hidden bg-white border border-orange-100 shadow-sm rounded-2xl">
                ...
                <div class="p-5">
                    ...
                    <div class="flex gap-3">
                        <form method="POST" action="{{ route('reimbursements.approve', $reimbursement) }}" class="flex-1">
                            @csrf
                        ...
                        <form method="POST" action="{{ route('reimbursements.reject', $reimbursement) }}" class="flex-1">
                            @csrf
        ...
        <div class="space-y-3">
            ...
            @if($reimbursement->status_1 === 'pending' && (int)$reimbursement->employee_id === (int)Auth::id())
            <a href="{{ route('reimbursements.edit', $reimbursement) }}"
                class="flex items-center gap-2 px-4 py-3 text-sm text-gray-700 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 shadow-sm">
            ...
            @endif
            <a href="{{ route('reimbursements.export-pdf', $reimbursement) }}"
                class="flex items-center gap-2 px-4 py-3 text-sm text-gray-700 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 shadow-sm">
