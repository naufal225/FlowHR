...
@section('subtitle', $permissions['canViewAllRequests'] ? 'Kelola semua pengajuan reimbursement' : 'Pengajuan reimbursement kamu')
...
<div class="overflow-hidden bg-white border border-gray-100 shadow-sm rounded-2xl">
    ...
    <div class="flex flex-col gap-4 px-5 py-4 border-b border-gray-100 bg-gray-50 sm:flex-row sm:items-center sm:justify-between">
        <h3 class="text-sm font-semibold text-gray-800">Daftar Reimbursement</h3>
        <div class="flex flex-wrap gap-2">
            ...
            <a href="{{ route('reimbursements.export', request()->query()) }}"
    ...
    {{-- Filters --}}
    <form method="GET" action="{{ route('reimbursements.index') }}" class="flex flex-wrap items-end gap-3 px-5 py-4 border-b border-gray-100">
        <div>
        ...
        <a href="{{ route('reimbursements.index') }}" class="px-4 py-1.5 text-sm text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50">Reset</a>
    </form>

    @if($permissions['canViewAllRequests'])
    <x-request-tabs />
    <div id="panel-my">
        @include('reimbursements._table', ['items' => $myReimbursements, 'showEmployee' => false])
    </div>
    <div id="panel-all" class="hidden">
        @include('reimbursements._table', ['items' => $allReimbursements, 'showEmployee' => true])
    </div>
    @else
    @include('reimbursements._table', ['items' => $myReimbursements, 'showEmployee' => false])
    @endif
...
@if($permissions['canMarkPayment'])
<form id="form-mark-done" method="POST" action="{{ route('reimbursements.marked') }}">
    @csrf @method('PATCH')
    <div id="mark-ids"></div>
</form>
@endif
