...
@section('header', 'Overtime')
@section('subtitle', $permissions['canViewAllRequests'] ? 'Kelola semua pengajuan overtime' : 'Pengajuan overtime kamu')
...
<div class="overflow-hidden bg-white border border-gray-100 shadow-sm rounded-2xl">
    ...
    <div class="flex flex-col gap-4 px-5 py-4 border-b border-gray-100 bg-gray-50 sm:flex-row sm:items-center sm:justify-between">
        <h3 class="text-sm font-semibold text-gray-800">Daftar Overtime</h3>
        <div class="flex flex-wrap gap-2">
            ...
            <a href="{{ route('overtimes.export', request()->query()) }}"
    ...
    {{-- Filters --}}
    <form method="GET" action="{{ route('overtimes.index') }}" class="flex flex-wrap items-end gap-3 px-5 py-4 border-b border-gray-100">
        <div>
        ...
        <a href="{{ route('overtimes.index') }}" class="px-4 py-1.5 text-sm text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50">Reset</a>
    </form>

    @if($permissions['canViewAllRequests'])
    <x-request-tabs />
    <div data-request-tab-panel="my">
        @include('overtimes._table', ['items' => $myOvertimes, 'showEmployee' => false])
    </div>
    <div data-request-tab-panel="all">
        @include('overtimes._table', ['items' => $allOvertimes, 'showEmployee' => true])
    </div>
    @else
    @include('overtimes._table', ['items' => $myOvertimes, 'showEmployee' => false])
    @endif
...
@if($permissions['canMarkPayment'])
<form id="form-mark-done" method="POST" action="{{ route('overtimes.marked') }}">
    @csrf @method('PATCH')
    <div id="mark-ids"></div>
</form>
@endif
