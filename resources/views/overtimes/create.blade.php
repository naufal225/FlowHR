@extends('layouts.app')
@section('title', 'Ajukan Overtime')
@section('header', 'Ajukan Overtime')
...
<div class="max-w-2xl mx-auto">
    ...
    <article class="overflow-hidden bg-white border border-gray-100 shadow-sm rounded-2xl">
        <header class="px-5 py-4 border-b border-gray-100 bg-gray-50">
            <h2 class="text-sm font-semibold text-gray-800">Form Pengajuan Overtime</h2>
        </header>
        <form method="POST" action="{{ route('overtimes.store') }}" class="p-5 space-y-4">
            @csrf
            <x-alert-errors />
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block mb-1 text-sm font-medium text-gray-700">Tanggal & Jam Mulai <span class="text-rose-500">*</span></label>
                    <input type="datetime-local" name="date_start" value="{{ old('date_start') }}" required
                ...
                <div>
                    ...
                    <input type="datetime-local" name="date_end" value="{{ old('date_end') }}" required
                        class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-300">
