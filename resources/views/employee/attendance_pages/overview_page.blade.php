...
@php
    $actionMap = [
        'not_checked_in' => ['title' => 'Check-in still pending', 'description' => 'Attendance scanning happens in the mobile app. Use the mobile attendance flow to record check-in for today.', 'icon' => 'fa-solid fa-mobile-screen-button', 'classes' => 'border-rose-200 bg-rose-50', 'iconClasses' => 'bg-rose-100 text-rose-700'],
        'checked_in' => ['title' => 'Check-out still pending', 'description' => 'Your check-in has been captured. Complete the day from mobile when your shift finishes.', 'icon' => 'fa-solid fa-hourglass-half', 'classes' => 'border-sky-200 bg-sky-50', 'iconClasses' => 'bg-sky-100 text-sky-700'],
        'complete' => ['title' => 'Attendance completed', 'description' => 'Today attendance is already complete. Use the history page if you need to review or audit an older record.', 'icon' => 'fa-solid fa-circle-check', 'classes' => 'border-emerald-200 bg-emerald-50', 'iconClasses' => 'bg-emerald-100 text-emerald-700'],
        'on_leave' => ['title' => 'Covered by leave', 'description' => 'You have an approved leave for today, so no attendance action is required.', 'icon' => 'fa-solid fa-umbrella-beach', 'classes' => 'border-slate-200 bg-slate-50', 'iconClasses' => 'bg-slate-100 text-slate-700'],
        'off_day' => ['title' => 'No attendance expected', 'description' => 'Today is marked as a non-working day, so attendance scanning is not required.', 'icon' => 'fa-solid fa-calendar-day', 'classes' => 'border-slate-200 bg-slate-50', 'iconClasses' => 'bg-slate-100 text-slate-700'],
        'absent' => ['title' => 'Attendance requires review', 'description' => 'No valid attendance record is available for today. Check your history and submit a correction only if the record is inaccurate.', 'icon' => 'fa-solid fa-triangle-exclamation', 'classes' => 'border-rose-200 bg-rose-50', 'iconClasses' => 'bg-rose-100 text-rose-700'],
        'config_issue' => ['title' => 'Attendance setup needs attention', 'description' => $todayState['description'], 'icon' => 'fa-solid fa-gear', 'classes' => 'border-red-200 bg-red-50', 'iconClasses' => 'bg-red-100 text-red-700'],
    ];
...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-[1.35fr_1fr]">
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            @else
                @include('components.attendance.empty-state', ['title' => 'Attendance policy is not available', 'description' => $policyError ?: 'Your assigned office does not have an active attendance policy yet.', 'icon' => 'fa-solid fa-gear'])
            @endif
        </div>
        ...
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div class="mt-5 space-y-3">
                @if($todayState['key'] === 'config_issue')
                    @include('components.attendance.state-panel', ['title' => 'Attendance setup issue', 'description' => $todayState['description'], 'icon' => 'fa-solid fa-gear', 'classes' => 'border-red-200 bg-red-50', 'iconClasses' => 'bg-red-100 text-red-700'])
                @elseif(collect($todayState['flags'])->pluck('key')->contains('suspicious'))
                    @include('components.attendance.state-panel', ['title' => 'Suspicious attendance detected', 'description' => 'The record for today was flagged as suspicious. Open the detail page or contact admin if you need this reviewed.', 'icon' => 'fa-solid fa-shield-halved', 'classes' => 'border-red-200 bg-red-50', 'iconClasses' => 'bg-red-100 text-red-700'])
                @else
                    @include('components.attendance.state-panel', ['title' => 'No critical issue', 'description' => 'Your today attendance state does not show any urgent warning at the moment.', 'icon' => 'fa-solid fa-circle-info', 'classes' => 'border-slate-200 bg-slate-50', 'iconClasses' => 'bg-slate-100 text-slate-700'])
                @endif
            </div>
        </div>
    </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-8">
                                @include('components.attendance.empty-state', ['title' => 'No attendance history yet', 'description' => 'Once you start using mobile attendance, the most recent records will appear here for quick review.', 'icon' => 'fa-solid fa-calendar-days', 'actionHref' => route('employee.attendance.history'), 'actionLabel' => 'Open full history'])
                            </td>
                        </tr>
                    @endforelse
                </tbody>
