        ...
        <div class="space-y-6">
            ...
            <div class="bg-white border rounded-xl shadow-soft border-neutral-200">
                ...
                <div class="p-6 space-y-3">
                    ...
                    <button onclick="window.location.href='{{ route('manager.overtimes.exportPdf', $overtime->id) }}'"
            ...
            <div class="bg-white border rounded-xl shadow-soft border-neutral-200">
                ...
                <div class="p-6">
                    ...
                    <div class="p-4 text-center text-green-800 rounded-lg bg-green-50">
                        ...
                        @if($overtime->note_2)
                        <p class="mt-2 text-sm"><strong>Notes:</strong> {{ $overtime->note_2 }}</p>
                        @endif
                    </div>
                    @elseif($overtime->status_2 === 'rejected')
                    <div class="p-4 text-center text-red-800 rounded-lg bg-red-50">
                        ...
                        @if($overtime->note_2)
                        <p class="mt-2 text-sm"><strong>Reason:</strong> {{ $overtime->note_2 }}</p>
                        @else
                        <p class="text-sm">No reason provided.</p>
                        @endif
                    </div>
                    @elseif($overtime->status_1 === 'approved' && $overtime->status_2 === 'pending')
                    <form id="approvalForm" method="POST" action="{{ route('manager.overtimes.update', $overtime) }}">
                        @csrf
                        @method('PUT')
...
<!-- Hidden Delete Form -->
<form id="delete-form-{{ $overtime->id }}" action="{{ route('manager.overtimes.destroy', $overtime->id) }}"
