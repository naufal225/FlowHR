                ...
                <form action="{{ route('super-admin.users.update', $user) }}" method="POST" class="space-y-6"
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 gap-3 p-4 border border-gray-300 rounded-lg bg-gray-50">
                            ...
                            <div class="flex items-center">
                                <input type="checkbox" name="roles[]" value="{{ $roleValue }}"
                                    ...
                                    @if($isEmployee) checked disabled @else {{ $isChecked ? 'checked' : '' }} @endif>
                                <label for="role_{{ $loop->index }}" class="ml-3 text-sm font-medium text-gray-700">
                                    {{ $label }}
                                </label>
                            </div>

                            {{-- Hidden field supaya Employee tetap ikut terkirim meski input disabled --}}
                            @if($isEmployee)
                        ...
                        <p class="mt-2 text-xs text-gray-500">
                            User can have multiple roles. At least one role is required.
                        </p>
                    </div>
