    ...
    <div class="max-w-2xl mx-auto">
        <div class="overflow-hidden bg-white border border-gray-100 shadow-sm rounded-xl">
            ...
            <div class="p-6">
                ...
                <form action="{{ route('super-admin.users.store') }}" method="POST" class="space-y-6" id="userForm">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 gap-3 p-4 border border-gray-300 rounded-lg bg-gray-50">
                        ...
                        $isEmployee = $roleValue === \App\Enums\Roles::Employee->value;
                        @endphp

                        <div class="flex items-center">
                            <input type="checkbox" name="roles[]" value="{{ $roleValue }}" id="role_{{ $loop->index }}"
                                class="w-4 h-4 text-blue-600 bg-white border-gray-300 rounded focus:ring-blue-500 focus:ring-2"
                                @if($isEmployee) checked disabled @else {{ in_array($roleValue, old('roles', $userRoles
                                ?? [])) ? 'checked' : '' }} @endif>
                        ...
                        {{-- Kalau pakai disabled, tambahkan hidden agar tetap terkirim --}}
                        @if($isEmployee)
                        <input type="hidden" name="roles[]" value="{{ $roleValue }}">
                        @endif
                        @endforeach
                    </div>
