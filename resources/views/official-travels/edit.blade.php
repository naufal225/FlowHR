@extends('layouts.app')
@section('title', 'Edit Perjalanan Dinas #' . $officialTravel->id)
@section('header', 'Edit Perjalanan Dinas')
...
<div class="max-w-2xl mx-auto">
    <nav class="mb-4 text-xs text-gray-500">
        ...
        <a href="{{ route('official-travels.show', $officialTravel) }}" class="hover:text-primary-600">Detail #{{ $officialTravel->id }}</a>
    ...
    <article class="overflow-hidden bg-white border border-gray-100 shadow-sm rounded-2xl">
        <header class="px-5 py-4 border-b border-gray-100 bg-gray-50">
            <h2 class="text-sm font-semibold text-gray-800">Edit Perjalanan Dinas #{{ $officialTravel->id }}</h2>
        </header>
        <form method="POST" action="{{ route('official-travels.update', $officialTravel) }}" class="p-5 space-y-4">
            @csrf @method('PUT')
            <x-alert-errors />
            <div>
                <label class="block mb-1 text-sm font-medium text-gray-700">Customer / Tujuan <span class="text-rose-500">*</span></label>
                <input type="text" name="customer" value="{{ old('customer', $officialTravel->customer) }}" required
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="date_start"
                        value="{{ old('date_start', \Carbon\Carbon::parse($officialTravel->date_start)->format('Y-m-d')) }}" required
                        class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-300">
                ...
                <div>
                    ...
                    <input type="date" name="date_end"
                        value="{{ old('date_end', \Carbon\Carbon::parse($officialTravel->date_end)->format('Y-m-d')) }}" required
                        class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-300">
            ...
            <div>
                ...
                <textarea name="description" rows="3" placeholder="Tujuan dan agenda perjalanan dinas (opsional)"
                    class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-300">{{ old('description', $officialTravel->description) }}</textarea>
                @error('description')<p class="mt-1 text-xs text-rose-500">{{ $message }}</p>@enderror
            </div>
            <div class="flex gap-3 pt-2">
                <a href="{{ route('official-travels.show', $officialTravel) }}"
