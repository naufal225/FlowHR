...
@section('content')
    <div class="max-w-3xl mx-auto">
        ...
        <nav class="flex mb-6" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        <i class="mx-2 fas fa-chevron-right text-neutral-400"></i>
                        <a href="{{ route('finance.reimbursements.show', $reimbursement->id) }}" class="text-sm font-medium text-neutral-700 hover:text-primary-600">Claim #RY{{ $reimbursement->id }}</a>
                    </div>
                </li>
        ...
        <div class="bg-white border rounded-xl shadow-soft border-neutral-200">
            ...
            <form action="{{ route('finance.reimbursements.update', $reimbursement->id) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    @if($reimbursement->invoice_path)
                        <div class="flex items-center mt-2 text-sm text-neutral-600">
                            ...
                            Current: <a href="{{ Storage::url($reimbursement->invoice_path) }}" target="_blank" class="ml-1 text-primary-600 hover:underline">View Invoice</a> {{-- Changed from attachment --}}
                            <label class="flex items-center ml-4">
                                <input type="checkbox" name="remove_invoice_path" value="1" class="rounded shadow-sm border-neutral-300 text-error-600 focus:ring-error-500"> {{-- Changed from remove_attachment --}}
